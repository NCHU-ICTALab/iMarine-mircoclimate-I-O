...
project: {version: v3.0, model_version: kaohsiung_port_dispatch_risk_v3.0}
...
port_local_training:
  ...
  label_alignment: {method: nearest_with_tolerance, tolerance_minutes: 30}
port_local_model: {enabled: true, train_wind_speed: true, train_wind_gust: true, train_rain_probability_if_labels_available: true}
model_selection:
  enabled: true
...
rain_probability: {enabled: true, preserve_in_all_modes: true, fallback_to_existing_model_if_port_local_unavailable: true, cwa_prior_enabled: true}
...
def test_predict_dispatch_risk_v30_falls_back_to_postprocess_and_preserves_rain(monkeypatch, tmp_path):
    ...
    result = predict_module.predict_dispatch_risk_v30(
    ...
    assert result["model_version"] == "kaohsiung_port_dispatch_risk_v3.0"
    assert result["model_selection_summary"]["selected_mode"] == "port_local_postprocess"
    assert result["model_selection_summary"]["dataset_ready"] is False
    assert result["trace"]["port_local_model_selected"] is False
    assert result["trace"]["fallback_to_port_local_postprocess"] is True
    assert result["trace"]["467441_used_as_core_station"] is False
    assert result["forecast_anchors"][0]["rain"]["final_probability"] == 0.31
    assert (tmp_path / "results" / "dispatch_risk_v30" / "model_selection_report.json").exists()
    assert (tmp_path / "results" / "dispatch_risk_v30" / "prediction_samples.json").exists()
